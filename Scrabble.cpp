...
void Scrabble::palabras_por_prefijo(const string& prefijo, const Dictionary& dictionary) {
    TrieNode* node = trie.search(prefijo); 

    if (!node) {
        cout << "//Prefijo inválido// Prefijo " << prefijo << " no pudo encontrarse en el diccionario." << endl;
    } else {
        cout << "//Resultado exitoso// Las palabras que comienzan con este prefijo son:" << endl;
        mostrarPalabrasNodo(node, prefijo, dictionary);
    }
}

void Scrabble::mostrarPalabrasNodo(TrieNode* node, const string& currentWord, const Dictionary& dictionary) {
    if (node->end_of_word) {
        cout << currentWord << " Longitud: " << currentWord.size() << ", Puntuación: " << dictionary.getWordScore(currentWord) << endl;
    }
    for (int i = 0; i < 26; ++i) {
        if (node->children[i] != nullptr) {
            mostrarPalabrasNodo(node->children[i], currentWord + char('a' + i), dictionary);
        }
    }
}

void Scrabble::palabras_por_sufijo(const string& sufijo, const Dictionary& dictionary) {
    string sufijoInvertido(sufijo.rbegin(), sufijo.rend()); 
    TrieNode* node = trie_inverse.search(sufijoInvertido);

    if (!node) {
        cout << "//Sufijo inválido// Sufijo " << sufijo << " no pudo encontrarse en el diccionario." << endl;
    } else {
        cout << "//Resultado exitoso// Las palabras que terminan con este sufijo son:" << endl;
        mostrarPalabrasInv(node, sufijoInvertido, dictionary);
    }
}

void Scrabble::mostrarPalabrasInv(TrieNode* node, const string& currentWord, const Dictionary& dictionary) {
    if (node->end_of_word) {
        string originalWord(currentWord.rbegin(), currentWord.rend()); 
        cout << originalWord << " Longitud: " << originalWord.size() << ", Puntuación: " << dictionary.getWordScore(originalWord) << endl;
    }
    for (int i = 0; i < 26; ++i) {
        if (node->children[i] != nullptr) {
            mostrarPalabrasInv(node->children[i], currentWord + char('a' + i), dictionary);
        }
    }
}
