<<<<<<< Updated upstream:ScrabbleGame/Scrabble.cpp
#include <iostream>
#include <vector>
#include <sstream>
#include <fstream>
...
void Scrabble::generarCombinaciones(const std::string& letras, std::string palabraActual, int pos, std::unordered_set<std::string>& posiblesPalabras, const Dictionary& dictionary) {
    if (pos == letras.size()) {
        if (!palabraActual.empty() && dictionary.wordExists(palabraActual)) {
    ...
    if (c == '?') {
        for (char letter = 'a'; letter <= 'z'; ++letter) {
            generarCombinaciones(letras, palabraActual + letter, pos + 1, posiblesPalabras, dictionary);
        }
    } else {
...
void Scrabble::posibles_palabras(std::string &let, const Dictionary& dictionary) {
    // Validar que las letras sean alfabéticas o un comodín
    if (!std::all_of(let.begin(), let.end(), [](char c) { return std::isalpha(c) || c == '?'; })) {
    ...
    auto words = this->graph.getWords(); // Obtener palabras del grafo

    // Función auxiliar para verificar si una palabra puede ser construida con las letras disponibles, incluyendo comodines
    auto canConstructWithWildcard = [](const std::string& letras, const std::string& palabra) {
        if (letras.size() != palabra.size()) return false;
        for (size_t i = 0; i < letras.size(); ++i) {
            if (letras[i] != '?' && letras[i] != palabra[i]) {
                return false;
            }
        }
        return true;
    ...
    // Iterar sobre cada palabra del grafo y verificar si puede ser construida
    for (const auto& word : words) {
        std::string wordStr = word.toString();
        if (canConstructWithWildcard(let, wordStr)) {
            posiblesPalabras.insert(word.toString());
        }
    }
    ...
    if (posiblesPalabras.empty()) {
        ...
        std::cout << "(Resultado exitoso) Las posibles palabras a construir con las letras " << let << " son:" << std::endl;
